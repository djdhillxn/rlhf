    ...
    def __call__(self, pairs):
        ...
        return {
            ...
            "weights": torch.tensor(
                [max(1.0, float(p.margin)) for p in pairs], dtype=torch.float32
            ),
...
def _refresh_reward_artifacts(output_dir):
...
def _load_step_offset_from_metrics(path):
...
def _maybe_clear_metric_artifacts(output_dir, enabled):
...
def evaluate_reward_model(model, loader, device, *, max_batches=None):
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total = 0
    ...
    with torch.no_grad():
        for step, batch in enumerate(loader):
            ...
            for domain, ok, margin in zip(
                domains,
                (diff > 0).detach().cpu().tolist(),
                diff.detach().cpu().tolist(),
            ):
                stats = by_domain.setdefault(
                    str(domain), {"correct": 0.0, "total": 0.0, "margin_sum": 0.0}
                )
...
def _optimizer_step(model, optimizer, max_grad_norm):
    if max_grad_norm > 0:
...
def run_reward_training(config_path, *, output_dir=None):
    cfg = load_config(config_path)
    output_dir = Path(
        output_dir
        or cfg.train.get("output_dir", "outputs/rlhf/qwen25_05b_helpsteer3_reward")
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "plots").mkdir(exist_ok=True)
    _maybe_clear_metric_artifacts(
        output_dir, bool(cfg.train.get("clear_existing_metrics", False))
    )
    ...
    if not val_pairs:
        raise RuntimeError(
            "No reward-model validation pairs were built from HelpSteer3."
        )
    save_jsonl(
        preference_pairs_to_dicts(train_pairs[: min(len(train_pairs), 1000)]),
        output_dir / "train_pairs_preview.jsonl",
    )
    save_jsonl(
        preference_pairs_to_dicts(val_pairs[: min(len(val_pairs), 1000)]),
        output_dir / "eval_pairs_preview.jsonl",
    )
    write_json(
        collect_run_metadata(
            run_type="rlhf_reward_model",
    ...
    if resume_checkpoint:
        ...
        if not resume_checkpoint.exists():
            raise FileNotFoundError(
                f"Reward resume checkpoint not found: {resume_checkpoint}"
            )
        ...
        if bool(cfg.model.get("gradient_checkpointing", True)) and hasattr(
    ...
    collator = PreferenceCollator(
        tokenizer, max_length=int(cfg.data.get("max_length", 1024))
    )
    ...
    val_loader = DataLoader(
        ...
        batch_size=int(
            cfg.train.get("eval_batch_size", cfg.train.get("batch_size", 2))
        ),
        shuffle=False,
        collate_fn=collator,
        num_workers=num_workers,
    ...
    if resume_step_offset in {None, "auto", "AUTO"} and cfg.train.get(
        "resume_from_checkpoint"
    ):
        resume_step_offset = _load_step_offset_from_metrics(
            Path(str(cfg.train.get("resume_from_checkpoint")))
        )
    ...
    for epoch in range(num_epochs):
        ...
        for local_step, batch in enumerate(pbar, start=1):
            ...
            batch_tokens = int(
                batch["chosen_attention_mask"].sum().item()
                + batch["rejected_attention_mask"].sum().item()
            )
            ...
            if pending_grads >= grad_accum:
                ...
                if global_step % log_every == 0:
                    ...
                    pbar.set_postfix(
                        ...
                        acc=f"{record['accuracy_batch']:.3f}",
                        ex_s=f"{record['examples_per_sec']:.1f}",
                    )

                if save_every_steps > 0 and global_step % save_every_steps == 0:
                    model.save_rlhf_pretrained(
        ...
        epoch_eval = evaluate_reward_model(
            model, val_loader, device, max_batches=cfg.train.get("eval_max_batches", 50)
        )
        epoch_eval.update(
            {
                "step": global_step,
        ...
        model.save_rlhf_pretrained(
            output_dir / f"checkpoint_epoch_{epoch + 1:02d}", tokenizer=tokenizer
        )
        _refresh_reward_artifacts(output_dir)

    final_metrics = evaluate_reward_model(
        model, val_loader, device, max_batches=cfg.train.get("final_eval_max_batches")
    )
